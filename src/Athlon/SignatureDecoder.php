...
class SignatureDecoder
{
    ...
    public function decode($signature, $js_code)
    {
        ...
        foreach ($instructions as $opt) {
            ...
            } elseif ('splice' == $command) {
                $signature = substr($signature, $value);
            } elseif ('reverse' == $command) {
    ...
    public function parseFunctionName($js_code)
    {
        if (preg_match('@,\s*encodeURIComponent\((\w{2})@is', $js_code, $matches)) {
            ...
            return $func_name;
        } elseif (preg_match('@(?:\b|[^a-zA-Z0-9$])([a-zA-Z0-9$]{2})\s*=\s*function\(\s*a\s*\)\s*{\s*a\s*=\s*a\.split\(\s*""\s*\)@is', $js_code, $matches)) {
    ...
    public function parseFunctionCode($func_name, $player_html)
    {
        ...
        if (preg_match('/'.$func_name.'=function\([a-z]+\){(.*?)}/', $player_html, $matches)) {
            ...
            // wm.vY(a,1);
            if (false != preg_match_all('/([a-z0-9$]{2})\.([a-z0-9]{2})\([^,]+,(\d+)\)/i', $js_code, $matches)) {
                // wm
                $obj_list = $matches[1];
                ...
                foreach ($matches2 as $m) {
                    if (false !== strpos($m[2], 'splice')) {
                        $functions[$m[1]] = 'splice';
                    } elseif (false !== strpos($m[2], 'a.length')) {
                        $functions[$m[1]] = 'swap';
                    } elseif (false !== strpos($m[2], 'reverse')) {
